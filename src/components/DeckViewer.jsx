...
const DeckViewer = () => {
  ...
  const questions = selectedTopic ? conversationData[selectedLevel][selectedGrammar][selectedTopic] : [];
  ...
  return (
    <div className="p-4 max-w-xl mx-auto">
      ...
      <div className="space-y-4">
        <select onChange={e => setSelectedLevel(e.target.value)} className="w-full p-2 border rounded">
          {levels.map(level => (
            <option key={level} value={level}>{level}</option>
          ))}
        </select>

        <select onChange={e => setSelectedGrammar(e.target.value)} className="w-full p-2 border rounded">
        ...
        <select onChange={e => setSelectedTopic(e.target.value)} className="w-full p-2 border rounded">
